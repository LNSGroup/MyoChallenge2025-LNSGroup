...
from .jax_utils import fix_repr, is_broadcastable
...
class BatchRenorm(hk.BatchNorm):
    ...
    def __call__(
      ...
      offset: jax.Array | None = None,
        ...
        return (inputs - custom_mean) * inv + offset
...
class QNetBN(hk.Module):
    ...
    def __call__(self, obs: jax.Array, act: jax.Array, is_training: bool = True
        ...
        input = jnp.concatenate((obs, act), axis=-1)
        return mlp_with_bn(input, self.hidden_sizes, 1, self.activation, self.output_activation, squeeze_output=True, 
...
@dataclass
class PolicyNetBN_DynSyn(hk.Module):
    muscle_dim: int
    group_dim: int
    hidden_sizes: Sequence[int]
    activation: Activation
    ...
    def __call__(self, obs: jax.Array, *, return_log_std: bool = False, is_training: bool = True,
        ...
        if jnp.ndim(obs) == 1:
            obs = obs.reshape(1, -1)
        if self.log_std_mode == 'shared':
            obs = BN(create_scale=True, create_offset=True, decay_rate=0.99, eps=1e-5)(obs, is_training=is_training)
            for hidden_size in self.hidden_sizes:
                obs = hk.Linear(hidden_size)(obs)
                obs = self.activation(obs)
                obs = BN(create_scale=True, create_offset=True, decay_rate=0.99, eps=1e-5)(obs, is_training=is_training)
            output = hk.Linear(self.group_dim * 2)(obs)
            output = self.output_activation(output)
            group_mean, group_log_std = jnp.split(output, 2, axis=-1)
        else:
            raise NotImplementedError(f"Log std mode '{self.log_std_mode}' not implemented in PolicyNetBN_WithLatent.")

        dynsyn_mean = hk.Linear(self.muscle_dim - self.group_dim)(obs)

        if not (self.min_log_std is None and self.max_log_std is None):
            group_log_std = jnp.clip(group_log_std, self.min_log_std, self.max_log_std)

        if return_log_std:
            return group_mean, group_log_std, dynsyn_mean
        else:
            return group_mean, jnp.exp(group_log_std), dynsyn_mean
